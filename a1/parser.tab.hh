...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == 0 || tok == 59 || tok == 43 || tok == 44 || tok == 40 || tok == 41 || tok == 123 || tok == 125 || tok == 91 || tok == 93 || tok == 58 || tok == 42 || tok == 60 || tok == 62 || tok == 45 || tok == 47 || tok == 33 || tok == 38 || tok == 61 || tok == 46);
      }
#else
      ...
      {
        YY_ASSERT (tok == token::STRING_LITERAL || tok == token::WHILE || tok == token::FOR || tok == token::STRUCT || tok == token::INT || tok == token::FLOAT || tok == token::VOID || tok == token::IF || tok == token::ELSE || tok == token::RETURN || tok == token::INC_OP || tok == token::OR_OP || tok == token::AND_OP || tok == token::PTR_OP || tok == token::EQ_OP || tok == token::NE_OP || tok == token::LE_OP || tok == token::GE_OP || tok == token::INT_CONSTANT || tok == token::FLOAT_CONSTANT || tok == token::IDENTIFIER);
      }
#else
