...
%code{
	...
	std::string code;
	extern vector<std::string> codegen;
	vector<std::string> readonly;
	int ro_label = 0;
...
function_definition:
	...
	compound_statement 
	{
		...
		//$3->gencode(code, gst, curr_st, curr, readonly, ro_label, param_size);
		//code += "\tmovl	$0, %eax\n";
...
main_definition:
	...
	{
		class LocalSymbTab* local_st = new class LocalSymbTab();
		gst.Entries[$2] = SymbolTableRow($2,"fun","global",0,0,Type($1,0,vector<int>()),local_st);
		curr_st = local_st;
		curr_trunit = "fun";
		curr = $2;
		func_map[$2] = vector<SymbolTableRow2>();
		code += "\t.text\n";
		code += "\t.globl\t"+curr+"\n";
		code += "\t.type\t"+curr+",\t@function\n";
		code += curr+":\n";
	...
	} ')' compound_statement 
	{
		...
		//$6->gencode(code, gst, curr_st, curr, readonly, ro_label, param_size);
		code += "\tmovl	$0, %eax\n";
		code += "\tleave\n";
		code += "\tret\n";
		code += "\t.size	main, .-main\n";
...
parameter_declaration:
	type_specifier declarator 
	{
		...
		{
			curr_st->table.push_back(SymbolTableRow2(curr_strow.symname,"var","param",dec_size*multi,8,global_type)); //to-be-changed
			func_map[curr].push_back(SymbolTableRow2(curr_strow.symname,"var","param",dec_size*multi,8,global_type));
			param_size += dec_size*multi;
		}
		else{
...
compound_statement:
	...
	{
		$$ = $2;
		$2->gencode(code, gst, curr_st, curr, readonly, ro_label, param_size);
	}
	;
...
primary_expression:
	IDENTIFIER 
	{
		...
		for (const auto entry : curr_st->table)
        {
            if(entry.symname == $1)
            {
                // offset = entry.offset;
...
declarator_list:
	declarator 
	{
		...
		if(curr.compare(0,6,"struct")!=0){
			code += "	subl	$"+to_string(dec_size*multi)+", %esp\n";
			//param_size += dec_size*multi;
		}
		dec_size = temp_dec_size;
		global_type = Type(global_type.type_name,0,vector<int>());
...
printf_call:
	...
	{
		string_astnode* string_node = new string_astnode($3);
		vector<exp_astnode*> expr_list;
		
		expr_list.push_back(string_node);

		identifier_astnode* idnode = new identifier_astnode($1);
		$$ = new proccall_astnode(idnode, expr_list);
